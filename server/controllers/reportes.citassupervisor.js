 ...
 * Cuenta citas por miembro del/los equipo(s) del supervisor autenticado (usa `inicio`).
 * Devuelve: { items:[{ejecutivoId, ejecutivo, total}], members:[{_id,name}] }
 */
export async function distribucionPorEjecutivoCitas(req, res) {
  try {
    ...
    res.status(500).json({ message: "Error generando reporte supervisor (citas)" });
  }
}
