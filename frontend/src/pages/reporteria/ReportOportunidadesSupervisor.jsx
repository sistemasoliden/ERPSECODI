...
export default function ReportOportunidadesSupervisor() {
  ...
  const loadBarsAndMembers = async () => {
    ...
    const { data } = await api.get("/reportes/oportunidades/por-ejecutivo", { ...authHeader, params: base });
  ...
  const loadSerieAndDist = async () => {
    ...
    try {
      ...
      const r1 = await api.get("/reportes/oportunidades/serie", { ...authHeader, params });
      const serieNorm = normalizeSerieDaysFull(r1.data?.items ?? r1.data, month, year);

      const r2 = await api.get("/reportes/oportunidades/distribucion", { ...authHeader, params });
    ...
    <div className="p-6 min-h-dvh bg-gradient-to-b from-slate-100 to-slate-50">
      <div className="mb-3 flex items-center justify-between gap-3">
        <h1 className="text-lg font-extrabold tracking-tight text-slate-900">
          Reportería • Oportunidades (Supervisor)
        </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        <Box title="Línea de progreso (Oportunidades por día)" className="lg:col-span-2">
      ...
      <div className="mt-3">
        <Box title="Oportunidades por Ejecutivo (equipo)" className="lg:col-span-3">
          {!bars.length ? <Empty /> : (
