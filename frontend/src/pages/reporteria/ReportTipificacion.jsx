...
function normalizeSerieDaysFull(data, month, year) {
  ...
  const map = new Map(
    arr
      .map((d) => {
        // Soporta:
        // - legacy: { day, total }
        // - rango:  { date:'YYYY-MM-DD', total }
        // - otros alias que ya usabas
        const dayRaw =
  ...
  // Construye el mes completo (1..31) del mes/año que tengas seleccionado
    ...
    <div className="p-6 min-h-dvh bg-gradient-to-b from-slate-100 to-slate-50">
      <div className="mb-3 flex items-center justify-between">
        <h1 className="text-lg font-extrabold tracking-tight text-slate-900">
          Reportería • Tipificación
        </h1>

        {/* SOLO filtro de rango */}
        <ReportRangeFilters
          from={from}
          to={to}
          minYear={2021}
          ...
          onChange={({ from: f, to: t }) => {
            ...
            // Si llega un 'from', usa ese mes/año para normalizar la serie del gráfico (eje 1..31)
            if (f) {
              const d = new Date(`${f}T00:00:00`);
              setMonth(d.getMonth() + 1);
              setYear(d.getFullYear());
            }
            // Si el usuario limpia el rango (ambos vacíos), vuelve al mes/año actual
            if (!f && !t) {
              const now = new Date();
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        <Box
          title="Línea de progreso (Tipificación por día)"
